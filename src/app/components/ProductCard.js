...
const ProductCard = ({
  ...
  status,
  productId,
  quantity, // Sirf ye prop add ki hai
}) => {
  ...
  return (
    ...
    >
      ...
      <div className="relative w-full aspect-[1/1.1] bg-[#edf1f5] overflow-hidden">
        ...
        {/* Status / Sold Out Badge - Floating Top-Left */}
        {(quantity <= 0 || status) && (
          <div className="absolute top-4 left-4 z-20">
            {quantity <= 0 ? (
              <span className="bg-[#0043EE] text-white text-[10px] font-black px-3 py-1.5 rounded-full uppercase tracking-tighter shadow-lg">
                Sold Out
              </span>
            ) : (
      ...
      <div className="p-5 flex flex-col flex-1 bg-white">
        ...
        <div className="flex gap-1.5 flex-wrap mb-2">
            <span className="text-[10px] font-bold bg-gray-50 text-gray-500 px-3 py-1.5 rounded-lg border border-[#0145f2]/10 group-hover:border-[#0145f2]/20 transition-all">
              {size}
            </span>
        </div>
